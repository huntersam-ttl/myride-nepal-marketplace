-- Security Sprint S3B
-- Critical + high-risk Supabase RLS / storage hardening
--
-- IMPORTANT:
-- - This migration is prepared for review only and should not be applied blindly.
...
-- =====================================================
-- 6. BLOG IMAGES: stop broad authenticated update/delete
-- =====================================================
...
DROP POLICY IF EXISTS "Admins or owners can update blog images" ON storage.objects;
DROP POLICY IF EXISTS "Admins or owners can delete blog images" ON storage.objects;

CREATE POLICY "Admins or owners can update blog images"
ON storage.objects
FOR UPDATE
TO authenticated
USING (
...
CREATE POLICY "Admins or owners can delete blog images"
ON storage.objects
FOR DELETE
TO authenticated
USING (
...
COMMENT ON POLICY "Admins or owners can update blog images" ON storage.objects IS
  'Authenticated users can no longer update arbitrary images in the shared blog-images bucket.';
